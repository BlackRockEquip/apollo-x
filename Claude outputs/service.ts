...
import { registerPexJob, syncPexConsumptionForJob, syncPexOnJobStatusChange } from "@/lib/pex/service";
...
export async function updateJob(ctx: RequestContext, id: string, raw: unknown) {
  ...
  const updated = await prisma.$transaction(async (tx) => {
    ...
    // Keep any PEX redeployment/status effects in step with whatever this
    // save just changed — both are no-ops unless the job is actually a
    // PEX Supply/Return job affected by this particular update.
    if (job.type === "PEX_SUPPLY") await syncPexConsumptionForJob(tx, ctx, companyId, job);
    if (input.status !== undefined && input.status !== existing.status) {
      await syncPexOnJobStatusChange(tx, ctx, companyId, job, existing.status as JobStatus, job.status);
    }
    return job;
  });
