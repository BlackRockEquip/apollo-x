...
export function DashboardSettingsWorkspace() {
  ...
  async function save() {
    ...
    try {
      const response = await fetch("/api/v1/dashboard", { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify(widgets) });
      const body = await response.json();
  ...
  return <div><div className="header-actions" style={{ marginBottom: 12 }}><button type="button" className="quiet-button" disabled={saving} onClick={() => void reset()}><RotateCcw size={14} /> Restore defaults</button><button type="button" className="gold-button" disabled={saving} onClick={() => void save()}><Save size={14} /> Save layout</button></div>{error ? <div className="inline-error">{error}</div> : null}{loading ? <div className="table-state">Loading…</div> : <section className="detail-panel"><header><div><h2>Available widgets</h2></div></header><div className="dashboard-widget-list">{ordered.map((widget, index) => <article key={widget.key}><strong>{available.find((row) => row.key === widget.key)?.label || widget.key}</strong><div className="stack-row"><label className="small-toggle"><input type="checkbox" checked={widget.enabled} onChange={(e) => setWidgets((current) => current.map((row) => row.key === widget.key ? { ...row, enabled: e.target.checked } : row))} /> Visible</label><button type="button" className="table-action" onClick={() => move(widget.key, -1)} disabled={index === 0}>↑</button><button type="button" className="table-action" onClick={() => move(widget.key, 1)} disabled={index === ordered.length - 1}>↓</button></div></article>)}</div></section>}</div>;
}
